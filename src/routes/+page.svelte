<script lang="ts">
    ...
    $: filteredInsignes = insignes
        ...
        .sort((a, b) => {
            switch (sortBy) {
                case "filiere": return toStringFiliere(a.filiere).localeCompare(toStringFiliere(b.filiere));
                case "categorie": return toStringCategorie(a.categories[0]).localeCompare(toStringCategorie(b.categories[0]));
                case "localisation": return localisationToString(a.localisation).localeCompare(localisationToString(b.localisation));
...
<div class="flex flex-col h-screen bg-gray-50">
    ...
    <header class="bg-white shadow-sm p-4 flex flex-wrap items-center gap-4 border-b border-gray-200 sticky top-0 z-10">
        ...
        <select bind:value={selectedFiliere} class="border border-gray-300 rounded-md px-2 py-1 text-sm">
            <option value="all">Toutes Filières</option>
            {#each Object.values(Filiere) as f (f)}
                {#if typeof f === "number"}<option value={f}>{toStringFiliere(f)}</option>{/if}
            {/each}
        </select>
        ...
        <select bind:value={selectedCategorie} class="border border-gray-300 rounded-md px-2 py-1 text-sm">
            ...
            {#each Object.values(Categorie) as c (c)}
                {#if typeof c === "number"}<option value={c}>{toStringCategorie(c)}</option>{/if}
            {/each}
        </select>
        ...
        <select bind:value={selectedLocalisation} class="border border-gray-300 rounded-md px-2 py-1 text-sm">
            ...
            {#each Object.values(Localisation) as l (l)}
                {#if typeof l === "number"}<option value={l}>{localisationToString(l)}</option>{/if}
            {/each}
        </select>
            ...
            {#if hovered}
                <div class="animate-in fade-in slide-in-from-right-2">
                    ...
                    <div class="text-sm text-gray-700 space-y-2">
                        <p><span class="font-medium text-gray-900">Catégories :</span> {hovered.categories.map(toStringCategorie).join(", ")}</p>
                        <p><span class="font-medium text-gray-900">Localisation :</span> {localisationToString(hovered.localisation)}</p>
                        <p><span class="font-medium text-gray-900">Filière :</span> {toStringFiliere(hovered.filiere)}</p>
                    </div>
                </div>
            {:else}
            ...
            <div class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 xl:grid-cols-8 gap-6">
                {#each filteredInsignes as insigne}
                    ...
                    >
                        ...
                        {/if}
                        <span class="text-sm text-gray-700 font-medium text-center truncate w-full block" title={insigne.nom}>
                        {insigne.nom}
                    </span>
                    </button>
                {/each}
